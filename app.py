...
# Hide the default Streamlit footer and menu for a cleaner look
hide_st_style = """
            <style>
...
# -------------------------------------------------
# 2. LOAD THE SAVED MODEL AND SCALER (Cached for speed)
# -------------------------------------------------
@st.cache_resource
...
# -------------------------------------------------
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/2917/2917995.png", width=80)  # Heart icon
...
with tab1:
    ...
    with col2:
        ...
        exang = st.selectbox("🏋️ Exercise Induced Angina", options=[0, 1], help="1=Yes, 0=No")
    ...
    if st.button("🔍 Predict Diagnosis", type="primary", use_container_width=True):
        ...
        # Display Results in a professional metric layout
...
# -------------------------------------------------
# TAB 2: BATCH CSV UPLOAD (INDUSTRY STANDARD)
# -------------------------------------------------
with tab2:
    ...
    # File uploader widget with professional icon/message
        ...
        try:
            ...
            if missing_cols:
                ...
                with st.spinner("🤖 Processing predictions..."):
                    ...
                    # Handle any missing values (replace with 0 just in case)
                    ...
                    m4.metric("⚠️ Risk Percentage", f"{(high_risk_count/total_patients)*100:.1f}%")
                    
                    # 2. Show the full results table (Interactive)
                    st.divider()
                    ...
                    st.dataframe(
                        df_results[display_cols].style.background_gradient(
                            subset=['Risk_Score'], cmap='RdYlGn_r', vmin=0, vmax=1
                        ),
                        use_container_width=True,
                        height=400
                    )
                    
                    # 3. Download button for results (Industry standard)
                    st.divider()
